            ...
            for(j=0;j<4000;j++)
            {
                if(cus[i].c_custkey==ord[j].o_custkey&&strcmp(ord[j].o_orderdate,order_date)<0)
                {
                    for(k=0;k<1000;k++)
                    {
                        ...
                        {
                            a[n].l_extendedprice=item[k].l_extendedprice;
                            strcpy(a[n].o_orderdate,ord[j].o_orderdate);
                            a[n].l_orderkey=item[k].l_orderkey;
                            //printf("找到了");
                            if(n>0 && a[n-1].l_orderkey==a[n].l_orderkey)
                            {
                                a[n-1].l_extendedprice+=a[n].l_extendedprice;
                                n--;
                                //printf("合并了");
                            }
                            n++;
 ...
 /*   for(i=0;i<n-1;i++)
    {
		...
		for(j=i+1;j<n;j++)
		{
			if(a[i].l_orderkey == a[j].l_orderkey && strcmp(a[i].o_orderdate,a[j].o_orderdate)==0)
				{
